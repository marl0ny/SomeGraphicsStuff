...
uniform complex complexPsiStepOffset;
...
complex xPsiStep2ndOrder(sampler2D tex) {
...
complex xPsiStep4thOrder(sampler2D tex) {
...
complex yPsiStep2ndOrder(sampler2D tex) {
    complex psiC = texture2D(tex, UV).zw;
...
complex yPsiStep4thOrder(sampler2D tex) {
...
complex psiStep2ndOrderNoSubstitution(sampler2D tex) {
    complex psiC0 = texture2D(tex, UV).zw;
    complex psiL1 = texture2D(tex, UV - vec2(dx/w, 0.0)).zw;
    complex psiR1 = texture2D(tex, UV + vec2(dx/w, 0.0)).zw;
    complex psiD1 = texture2D(tex, UV - vec2(0.0, dy/h)).zw;
...
complex psiStep4thOrderNoSubstitution(sampler2D tex) {
    complex psiC0 = texture2D(tex, UV).zw;
    complex psiL2 = texture2D(tex, UV - 2.0*vec2(dx/w, 0.0)).zw;
    complex psiL1 = texture2D(tex, UV - vec2(dx/w, 0.0)).zw;
    complex psiR1 = texture2D(tex, UV + vec2(dx/w, 0.0)).zw;
...
complex nonlinear(float magnitude, sampler2D tex) {
    ...
    complex psi = texture2D(tex, UV).zw;
    return magnitude*(0.0 - absSquared(psi))*psi;
}
...
vec2 vectorPotentialStep(sampler2D tex) {
    ...
    complex xGradPsi = ((psiL2 - psiR2)/12.0 + 2.0*(psiR1 - psiL1)/3.0)/dx;
    complex yGradPsi = ((psiD2 - psiU2)/12.0 + 2.0*(psiU1 - psiD1)/3.0)/dy;
    vec2 current = vec2(real(mul(conj(psiC0), mul(IMAG_UNIT, xGradPsi))
                             + A.x*mul(conj(psiC0), psiC0)),
                        real(mul(conj(psiC0), mul(IMAG_UNIT, yGradPsi))
                             + A.y*mul(conj(psiC0), psiC0)));
...
void main() {
    fragColor = vec4(vectorPotentialStep(tex)/sigma,
                     mul(complexPsiStepOffset,
                         // psiStep4thOrderNoSubstitution(tex)
                         xPsiStep4thOrder(tex) + yPsiStep4thOrder(tex)
                         // -xPsiStep2ndOrder(tex) - yPsiStep2ndOrder(tex)
                         + nonlinear(kappa, tex))
                     );
}
