...
/* void init_sum_quads(struct TextureParams tex_params,
                    frame_id *sum_quads, size_t n_sum_quads) {
    pow();
    for (int size = n_sum_quads; )
}*/
...
struct Vec4 texture_reduction_sum(int scale_program,
    ...
    struct Vec4 arr[4];
    get_quad_texture_array(sum_quads[i-1], 0, 0, 2, 2, GL_FLOAT, arr);
