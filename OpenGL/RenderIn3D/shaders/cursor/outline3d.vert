...
void main() {
    vec3 position2 = vec3(
        (position.x == 0.0)? cursorPosition.x: position.x,
        (position.y == 0.0)? cursorPosition.y: position.y,
        (position.z == 0.0)? cursorPosition.z: position.z
    );
    vec3 r = rotate(quaternion(position2.xyz, 1.0), 
