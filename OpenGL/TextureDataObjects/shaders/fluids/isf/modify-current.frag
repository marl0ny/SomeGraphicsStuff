#VERSION_NUMBER_PLACEHOLDER

#if (__VERSION__ >= 330) || (defined(GL_ES) && __VERSION__ >= 300)
#define texture2D texture
#else
#define texture texture2D
#endif

#if (__VERSION__ > 120) || defined(GL_ES)
precision highp float;
#endif
 
#if __VERSION__ <= 120
varying vec2 UV;
#define fragColor gl_FragColor
#else
in vec2 UV;
out vec4 fragColor;
#endif

#define complex vec2
#define complex2 vec4

#define PI 3.141592653589793

uniform float radius;
uniform vec2 r0;
uniform ivec2 wavenumber;
uniform sampler2D waveFuncTex;

complex mul(complex z1, complex z2) {
    return complex(z1.x*z2.x - z1.y*z2.y, 
                   z1.x*z2.y + z1.y*z2.x);
}

void main() {
    float u = UV[0] - r0[0];
    float v = UV[1] - r0[1];
    float r = sqrt(u*u + v*v);
    float nx = float(wavenumber[0]);
    float ny = float(wavenumber[1]);
    complex phase = complex(cos(2.0*PI*(nx*u + ny*v)),
                            sin(2.0*PI*(nx*u + ny*v)));
    complex2 initWaveFunc = texture2D(waveFuncTex, UV);
    complex initPsi0 = initWaveFunc.xy;
    complex initPsi1 = initWaveFunc.zw;
    fragColor = vec4((r < radius)? phase: initPsi0,
                     (r < radius)? phase: initPsi1);
}
