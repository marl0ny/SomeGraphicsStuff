...
int main() {
    ...
    unsigned char *image = get_bitmap_contents(
        "./textures/wood-texture.bmp", &image_w, &image_h);
    ...
    vertices_init(&v, 2*numberOfPointsPerEdge*6, buff, NULL);
    make_cylinder(&v, 0.5, 0.125, numberOfPointsPerEdge);

    /*float *buff2 = (float *)buff + v.n*v.vertex_size;
    struct vertices v2;
    vertices_init(&v2, 2*numberOfPointsPerEdge*6, buff2, NULL);
    make_cylinder(&v2, 0.5, 0.125, numberOfPointsPerEdge);
    float translate1[3] = {0.5, 0.0, 0.5};
    vertices_translate_position(&v2, translate1);
    float *buff3 = (float *)buff2 + v2.n*v2.vertex_size;
    struct vertices v3;
    vertices_init(&v3, 2*numberOfPointsPerEdge*6, buff3, NULL);
    make_cylinder(&v3, 0.5, 0.125, numberOfPointsPerEdge);
    float translate2[3] = {0.5, 0.0, -0.5};
    vertices_translate_position(&v3, translate2);
    float *buff4 = (float *)buff3 + v3.n*v3.vertex_size;
    struct vertices v4;
    vertices_init(&v4, 2*numberOfPointsPerEdge*6, buff4, NULL);
    make_cylinder(&v4, 0.5, 0.125, numberOfPointsPerEdge);
    float translate3[3] = {-0.5, 0.0, -0.5};
    vertices_translate_position(&v4, translate3);
    float *buff5 = (float *)buff4 + v4.n*v4.vertex_size;
    struct vertices v5;
    vertices_init(&v5, 2*numberOfPointsPerEdge*6, buff5, NULL);
    make_cylinder(&v5, 0.5, 0.125, numberOfPointsPerEdge);
    float translate4[3] = {-0.5, 0.0, 0.5};
    vertices_translate_position(&v5, translate4);
    numberOfPointsPerEdge = 8;
    float *buff6 = (float *)buff5 + v5.n*v5.vertex_size;
    struct vertices v6;
    vertices_init(&v6, 2*numberOfPointsPerEdge*6, buff6, NULL);
    make_cylinder(&v6, 0.1, 0.9, numberOfPointsPerEdge);
    float translate5[3] = {0.0, -0.25-0.05, 0.0};
    vertices_translate_position(&v6, translate5);*/
    ...
    glTextureParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST_MIPMAP_LINEAR);
