...
void main() {
    vec2 uv = uvIndex.xy;
    vec4 pos = vec4(to3DTextureCoordinates(uv), 1.0)
               - vec4(0.5, 0.5, 0.5, 0.0);
    vec4 pos1 = rotate(pos, quaternionConjugate(rotation))
                + vec4(0.5, 0.5, 0.5, 0.0);
    POSITION = pos1.xyz;
    gl_Position = project(2.0*scale*rotate(pos, debugRotation)); // + vec4(translate, 0.0);
}
