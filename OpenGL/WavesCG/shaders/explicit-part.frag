...
uniform sampler2D waveTex0;
uniform sampler2D waveTex1;
...
void main() {
    ...
    vec4 w0 = texture2D(waveTex0, UV);
    vec4 w1 = texture2D(waveTex1, UV);
    vec4 del2W0 = laplacian2ndOrder4Point(waveTex0);
    fragColor = dt*dt*del2W0/2.0 + dt*dt*f
                + 2.0*rho*w1 + (dt*rho*gamma/2.0 - rho)*w0;
}