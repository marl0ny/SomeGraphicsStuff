...
def copy_file_contents(dest: str, src: str, 
    ...
    a destination file.
    """
    start_key: str = 'START_OF_FILE' if start_key is None else start_key
    end_key: str = 'END_OF_FILE' if end_key is None else end_key
    dest_file_list: list = [[], [], []]
    before, inside, after = 0, 1, 2
    ...
    with open(dest, "r") as dest_file:
        for line in dest_file:
            if start_key in line and state == before:
                ...
                state = inside
            elif end_key in line and state == inside:
    ...
    dest_file_list[inside] = [f'// {start_key} \n']
    if src != "":
        with open(src, "r") as src_file:
            for line in src_file:
                dest_file_list[inside].append(line)
    dest_file_list[inside].append('\n')
...
if __name__ == "__main__":
    target_file: str = "./scripts/shader-sources.js"
    vertex_file: str = "./shaders/vertices.vert"
    fragment_file: str = "./shaders/fragments.frag"
    if len(sys.argv) == 1:
        ...
        port: int = int(random.random()*50000) + 1024
        thread: LaunchNewBrowserThread = LaunchNewBrowserThread()
        thread.start()
